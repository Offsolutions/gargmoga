...
class DataOperation extends Database
{
    ...
    function getDataJoin($table, $star, $join)
    {

        $record = array();
        $sql = "select $star from $table $join";
        $result = $this->conn->prepare($sql);
        $result->execute();
        if ($result->rowCount() > 0) {
            while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
    ...
    function selectDataJoin($table, $where, $star, $join)
    {
        $sql = "";
        $record = array();
        $condition = "";
        ...
        $condition = substr($condition, 0, -5);
        $sql .= "select $star from $table $join where $condition";
        $result = $this->conn->prepare($sql);
        $result->execute(array_values($where));
        if ($result->rowCount() > 0) {
            $row = $result->fetch(PDO::FETCH_ASSOC);
    ...
    function selectDataAllJoin($table, $where, $star, $join)
    {
        $sql = "";
        $record = array();
        $condition = "";
...
if (isset($_POST["add_banners"])) {
    if (!empty($_POST["title"])) {
        if (!empty($_FILES["img"]["name"])) {
            $img = $obj->imageUploadThumb($_FILES["img"]["tmp_name"], $_FILES["img"]["name"], 1920, 1280);
            if ($img) {
                $_POST["img"] = $img;
            }
        }
